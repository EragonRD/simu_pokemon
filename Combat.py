...
class Combat:
    ...
    def lance_attaque(self, attaque):
        if (self.tour):
            self.info = self.poke_2.get_nom() + " attaque avec {attaque}"
            lance_attack(attaque, self.poke_2, self.poke_1)
        else :
            self.info = self.poke_1.get_nom() + " attaque avec {attaque}"
            lance_attack(attaque, self.poke_1, self.poke_2)
        self.tour = 1 - self.tour

    

def lance_attack(attaque, p1, p2):
    """
...
def lanceAttack(attaquant :type[Pokemon],defenseur: type[Pokemon]):
        ...
        if capacite.categorie == 0 and capacite.cible == 1 :
            defenseur.vie = defenseur.vie - math.floor((math.floor(math.floor(((math.floor(attaquant.lvl * 0.4 + 2) * attaquant.attack * capacite.get_puissance())/attaquant.defense)/50)) +2)*cm)
        # attaque spéciale
        if capacite.categorie == 1 and capacite.cible == 1 : 
            defenseur.vie = defenseur.vie - math.floor((math.floor(math.floor(((math.floor(attaquant.lvl * 0.4 + 2) * attaquant.attSpecial * capacite.get_puissance())/attaquant.defSpecial)/50)) +2)*cm)
